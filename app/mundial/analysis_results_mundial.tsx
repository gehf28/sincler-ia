...
import { Trophy, Goal, Activity, Sparkles, HelpCircle, ShieldCheck, Star, RefreshCw, Award, CheckCircle2, Share2 } from 'lucide-react'
import { toPng } from 'html-to-image';

export default function AnalysisResultsMundial({ data }: { data: any }) {
  ...
  const handleShareTicket = async () => {
    const node = document.getElementById('ticket-container-mundial');
    if (node) {
      try {
        const dataUrl = await toPng(node, { backgroundColor: '#0b0f19' });
        const response = await fetch(dataUrl);
        const blob = await response.blob();
        const file = new File([blob], 'analisis-mundial.png', { type: 'image/png' });
        if (navigator.share && navigator.canShare({ files: [file] })) {
          await navigator.share({ files: [file], title: 'Análisis Sincler Mundial', text: 'Mira este pronóstico' });
        } else {
          const link = document.createElement('a');
          link.download = 'analisis-mundial.png';
          link.href = dataUrl;
          link.click();
        }
  ...
  return (
    <div className="flex flex-col gap-4 w-full mt-8">
      <div id="ticket-container-mundial" className="w-full bg-[#0b0f19] rounded-3xl p-6 md:p-8 border-2 border-cyan-500/30 shadow-2xl animate-fade-in relative overflow-hidden text-slate-100">
        
        {/* Encabezado - MANTENIENDO TU DISEÑO ORIGINAL */}
        <div className="flex flex-wrap items-center justify-between gap-4 border-b border-slate-800 pb-6 mb-6">
          <div className="flex items-center gap-2 bg-cyan-500/10 text-cyan-400 px-4 py-1.5 rounded-full text-xs font-black uppercase tracking-wider border border-cyan-500/20">
            <Sparkles className="w-4 h-4 fill-current" /> Algoritmo Copa del Mundo 2026
          </div>
          
          <div className="flex items-center gap-3">
            {/* Fecha Integrada */}
            {/* Usamos || new Date() para que si no hay fecha guardada, muestre la actual */}
            {(data.fecha_analisis || new Date()) && (
              <div className="text-[10px] font-black text-cyan-400 uppercase tracking-widest bg-cyan-500/10 px-3 py-1.5 rounded-lg border border-cyan-500/30 shadow-[0_0_10px_rgba(34,211,238,0.1)]">
                {new Date(data.fecha_analisis || new Date()).toLocaleString('es-ES', { 
                  day: 'numeric', 
                  month: 'short', 
                  hour: '2-digit', 
                  minute: '2-digit' 
            ...
            <div className="text-xs font-black uppercase px-3 py-1.5 rounded-xl bg-emerald-500/10 text-emerald-400 border border-emerald-500/30 flex items-center gap-1.5">
              <CheckCircle2 className="w-4 h-4" /> Predicción Procesada
            </div>
          </div>
        </div>
        ...
        <div className="bg-[#131926]/40 border border-slate-800/80 p-6 rounded-2xl mb-6">
          ...
          <div className="flex justify-between font-black text-sm md:text-base mb-2">
            <span className={data.favorito === data.team1 ? 'text-cyan-400' : 'text-white'}>{data.team1} ({probTeam1}%)</span>
            <span className="text-slate-400 text-xs md:text-sm">Empate ({probEmpate}%)</span>
            <span className={data.favorito === data.team2 ? 'text-cyan-400' : 'text-white'}>{data.team2} ({probTeam2}%)</span>
          </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-4">
          <div className="bg-[#131926]/60 border border-slate-800 p-4 rounded-2xl flex flex-col justify-between">
            <div className="flex items-start justify-between w-full mb-2">
              <div className="flex items-center gap-3">
                <div className="w-9 h-9 rounded-xl bg-cyan-500/10 flex items-center justify-center text-cyan-400"><Trophy className="w-4 h-4" /></div>
                <div>
            ...
            <p className="text-xs text-slate-300 italic mt-1 leading-snug">"{data.ganador?.explicacion || 'Procesando...'}"</p>
          </div>

          <div className="bg-[#131926]/60 border border-slate-800 p-4 rounded-2xl flex flex-col justify-between">
            <div className="flex items-start justify-between w-full mb-2">
              <div className="flex items-center gap-3">
                <div className="w-9 h-9 rounded-xl bg-[#00ff87]/10 flex items-center justify-center text-[#00ff87]"><Goal className="w-4 h-4" /></div>
                <div>
            ...
            <p className="text-xs text-slate-300 italic mt-1 leading-snug">"{data.goles?.explicacion || 'Procesando...'}"</p>
          </div>
        </div>
        ...
        <div className="border-t border-slate-800 pt-6 mb-6">
          <p className="text-[11px] font-black text-cyan-400 uppercase tracking-widest mb-4 flex items-center gap-1.5">
            ...
            Métricas Avanzadas de Torneo Corto
          </p>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div className="bg-[#131926]/50 border border-slate-800/60 p-5 rounded-2xl flex flex-col min-h-[160px]">
              <div className="flex items-center gap-2 text-xs font-bold text-slate-400 uppercase tracking-wider mb-2"><Award className="w-4 h-4 text-[#00ff87]" /> Quién Clasifica</div>
              <div className="flex-1 flex items-center justify-center"><p className="text-xl font-black text-white text-center">{data.quienClasifica || "Simulando..."}</p></div>
            </div>
            <div className="bg-[#131926]/50 border border-slate-800/60 p-5 rounded-2xl min-h-[160px]">
              ...
              <div className="space-y-2">{data.razonesClave?.map((r: string, i: number) => <p key={i} className="text-xs font-semibold text-slate-300 leading-tight">{r}</p>)}</div>
            </div>
          </div>
        </div>
        ...
        <div className="bg-[#0b0f19] border border-slate-800 rounded-2xl p-5">
          <p className="text-xs font-black text-cyan-400 uppercase tracking-widest mb-2 flex items-center gap-1.5"><HelpCircle className="w-4 h-4" /> Dictamen Estratégico Mundialista</p>
          <p className="text-sm font-medium leading-relaxed text-slate-200">{data.comentario_general || 'Procesando veredicto...'}</p>
        </div>
      </div>
